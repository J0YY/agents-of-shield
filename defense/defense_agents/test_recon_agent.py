...
The agent uses the log-reader-mcp server to read network traffic.
"""
...
# log-reader-mcp expects logs/logs.log in the working directory
...
def main():
    ...
    print("📡 Using log-reader-mcp library to read network traffic...")
    print(f"📁 Working directory: {WORKING_DIR}")
    print(f"📁 Expected log file: {WORKING_DIR / 'logs' / 'logs.log'}")
    print()
    
    # Check if log file exists
    log_file = WORKING_DIR / "logs" / "logs.log"
    if not log_file.exists():
        print(f"⚠️  Warning: Log file not found at {log_file}")
        print("   Make sure the vulnerable app is logging to logs/logs.log")
        print("   You may need to create the logs directory first:")
        print(f"   mkdir -p {WORKING_DIR / 'logs'}")
        print()
    
    agent = ReconAgent(working_dir=WORKING_DIR)
    
    print("🚀 Starting investigation...")
    print("   (This may take a moment as it connects to log-reader-mcp and analyzes logs)")
    print()
    ...
    try:
        ...
        print("1. Make sure log-reader-mcp is installed: npx log-reader-mcp init")
        print("2. Check that logs/logs.log exists and contains log entries")
        print("3. Verify OPENAI_API_KEY is set correctly")
        print("4. Ensure the agents framework is installed: pip install openai-agents")
        import traceback
        traceback.print_exc()
